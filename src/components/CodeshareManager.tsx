  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
      <form onSubmit={add} className="rounded-2xl border border-obsidian bg-ink-900 p-6">
        ...
        <p className="mt-1 text-sm text-cream-dim">New routes appear instantly in the pilot route database and oneworld Discover.</p>
        <div className="mt-5 space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <div><label className={label}>From</label><input className={`${input} font-mono uppercase`} maxLength={4} value={f.dep} onChange={(e) => set("dep", e.target.value)} placeholder="EFHK" /></div>
      ...
      <div>
        ...
        <div className="mt-4 space-y-2">
          {routes.length === 0 && <div className="rounded-2xl border border-dashed border-obsidian bg-ink-900 p-8 text-center text-sm text-cream-faint">No codeshare routes added yet. The base network already includes oneworld partners.</div>}
          {routes.map((r) => (
