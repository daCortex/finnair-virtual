...
/* Finnair route pairs ("DEP|ARR") → the aircraft tokens flown on them. Built
   once from the route database; a live flight must match one to be shown. */
const FINNAIR_ROUTES = (() => {
...
// Finnair Virtual flights airborne right now (callsign contains "Finnair"),
...
export async function GET() {
  ...
  const [flights, liveries] = await Promise.all([
    getFlightsByCallsignSuffix("FINNAIR"),
    getLiveryMap(),
  ]);
  ...
  const out = flights
    .map((f, i) => {
      ...
      const allowed = FINNAIR_ROUTES.get(`${origin}|${dest}`);
