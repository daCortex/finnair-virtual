...
async function handleRegister(interaction: Interaction) {
  ...
  if (callsign.length < 2) {
    return message("Please provide your Finnair callsign, e.g. `Finnair 042`.", {
      ephemeral: true,
    });
  }
  if (ifc.length < 2) {
    ...
    embeds: [
      {
        ...
        footer: {
          text: reclaimed
            ...
            : "Welcome to Finnair Virtual — The Nordic Way to Fly.",
        },
      },
    ],
  });
...
async function handleStats(interaction: Interaction) {
  ...
  if (!pilot) {
    return message(
      userId || callsignOpt
        ? "No pilot found — check the callsign (e.g. `Finnair 042`)."
        : "You're not on the roster yet — use `/register` to join.",
      { ephemeral: true },
    );
  }

  // Public Finnair Virtual stats card (visible to everyone in the channel).
...
async function aboutSvEmbed(pilot: any) {
  ...
  return {
    title: `${pilot.callsign} — Finnair Virtual`,
    color: GOLD,
    fields,
...
/* Toggle the /aboutme card between Finnair Virtual and Infinite Flight stats. */
