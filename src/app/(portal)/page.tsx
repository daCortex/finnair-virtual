    ...
    <div className="mx-auto max-w-6xl px-5 py-8 lg:px-8 lg:py-10">
      ...
      <section className="rise overflow-hidden rounded-3xl border border-obsidian">
        <div className="aurora relative px-6 py-8 lg:px-10">
          <div className="pointer-events-none absolute -right-20 -top-20 h-72 w-72 rounded-full opacity-30 blur-3xl" style={{ background: "radial-gradient(circle, #c451d6, transparent 70%)" }} />
      ...
      <section className="mt-4 grid gap-4 lg:grid-cols-3">
        ...
        <div className="rise overflow-hidden rounded-2xl border border-obsidian bg-ink-900 lift lg:col-span-2">
          ...
          <div className="flex flex-col gap-5 px-6 py-6 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1.5 text-sm text-cream-faint">{firstFlightNo(rotw)} · {rotw.aircraft.replace(/^Finnair |^Nordic Regional /, "")} · {fmtHours(rotw.minutes)}</p>
            </div>
        ...
        <div className="rise rounded-2xl border border-obsidian bg-ink-900 lift">
          <div className="flex items-center justify-between border-b border-obsidian/70 px-5 py-3">
            ...
            <Link href="/career" className="text-xs text-gold hover:underline">All →</Link>
          </div>
          {d.gates.career ? (
            ...
            <div className="px-5 py-6 text-center">
              <p className="text-sm text-cream-dim">Career Mode unlocks at <span className="font-medium text-cream">Elysian</span></p>
              <p className="mt-1 text-xs text-cream-faint">{Math.max(0, Math.ceil(75 - d.totalHours))} flight hours to go</p>
            </div>
          )}
        </div>
      </section>
      ...
      {events.length > 0 && (
        <section className="mt-4 rounded-2xl border border-obsidian bg-ink-900 px-5 py-4">
          <div className="flex items-center justify-between">
            ...
            <Link href="/routes" className="text-xs text-cream-faint hover:text-cream">View all</Link>
          </div>
      ...
      <section className="mt-4">
        <div className="flex flex-wrap gap-2">
          <Pill href="/routes" label="Route database" />
          <Pill href="/map" label="Live map" />
          <Pill href="/logbook" label="Logbook" />
          <Pill href="/cargo" label="Cargo" locked={!d.gates.cargo} />
          <Pill href="/special-ops" label="Special Ops" locked={!d.gates.specialOps} />
          <Pill href="/discover" label="oneworld Discover" locked={!d.gates.discover} />
        </div>
      </section>
    </div>
  );
