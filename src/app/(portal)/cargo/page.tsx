...
export default async function CargoPage() {
  ...
  if (!d) return null;
  if (!d.gates.cargo) {
    return <Locked title="Logistics Command" rank="Zenith" hours={300} current={d.totalHours} blurb="The Cargo track — freight contracts paid in Logistics Credits (LC) — opens at Zenith, the peak of the core ladder." accent="rose" />;
  }
        ...
        {contracts.map((c, i) => (
          <div key={c.id} className="rise flex flex-col rounded-2xl border border-obsidian bg-ink-900 p-5 lift" style={{ animationDelay: `${i * 60}ms` }}>
            ...
            <div className="mt-auto flex items-center justify-between border-t border-obsidian/70 pt-3.5">
              ...
              <Link href="/file" className="rounded-full px-4 py-2 text-xs font-semibold text-white transition-all hover:brightness-125" style={{ background: "var(--color-rose)" }}>Accept</Link>
            </div>
          </div>
        ))}
      </div>
