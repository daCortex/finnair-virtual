/* ----------------------------------------------------------------
   Fileable aircraft — our own Finnair fleet PLUS every aircraft flown
   ...
   log codeshare flights, not just the three Finnair Airbus types.
------------------------------------------------------------------- */
...
export const FINNAIR_AIRCRAFT: string[] = FLEET.map((a) => a.type);
...
export const AIRCRAFT_GROUPS: { group: string; items: string[] }[] = [
  { group: "Finnair fleet", items: FINNAIR_AIRCRAFT },
...
/* Best aircraft value to prefill when a route is chosen: the specific
   Finnair fleet type for our own routes, the clean model for codeshares. */
