/* ----------------------------------------------------------------
   Finnair Virtual — pilot portal aggregation.
   Builds the dashboard model for the signed-in pilot: hours, PIREPs,
   rank + licence progression, Aurora-Points balance, Finnair Plus tier
   and the feature gates (Career / Cargo / Special Ops / oneworld Discover).
------------------------------------------------------------------- */
...
export async function getPilotDashboard(): Promise<PilotDashboard | null> {
  ...
  const gates: Gates = {
    ...
    discover: hasRankAtLeast(totalHours, GATES.oneworldDiscover),
  };
