...
/* Active flights on the Expert Server whose callsign contains `match`
   (e.g. "FINNAIR" → Finnair Virtual flights). Expert-only by design. */
...
function isAirport(id: string | null): boolean {
  // ICAO airport identifiers are 3–4 letters (KSEA, EFHK, EGLL, OMDB…).
...
/* Recent Finnair flights from a pilot's IF logbook, newest first —
...
export async function getRecentFinnairFlights(
  userId: string,
  limit = 8,
  ...
  return flights
    .filter((f) => (f.callsign ?? "").toUpperCase().trim().includes("FINNAIR"))
    .slice(0, limit)
    .map((f) => {
